    ...
    var settings = $.extend({
		...
		if (supportedBrowser) {
			if(isFeedbackButtonNative) {
				$('.quick-access').append('<button class="feedback-btn feedback-btn-gray">' + settings.initButtonText + '</button>');
			}
			$(document).on('click', settings.feedbackButton, function(){
				...
				$(document).on('click', '#feedback-highlighter-next', function() {
					...
					html2canvas($('body'), {
						onrendered: function(canvas) {
							...
							img = _canvas.get(0).toDataURL("image/jpeg");
				...
				$(document).on('click', '#feedback-submit', function() {
					...
					if ($('#feedback-note').val().length > 0) {
                        ...
                        var data = {feedback: JSON.stringify(post), _token: settings.csrfToken};
						$.ajax({
